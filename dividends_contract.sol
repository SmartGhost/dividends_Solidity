...
contract StandardToken is Token {
    ...
    address public teamHolder= 0x0;  
    uint256 public holdersBalance;
    uint public totalHolders;
    uint public timeToFreeze = 1;  
    ...
    function transfer(address _to, uint256 _value) returns (bool success) {
        if(msg.sender == owner && (balances[msg.sender] - _value) < 30){ 
             { return false; }
        }
        if(msg.sender == teamHolder && timeToFreeze < now ){
             { return false; }
        }
